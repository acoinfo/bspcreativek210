...
int is_memory(uintptr_t address)
{
    enum {
        mem_len = 6 * 1024 * 1024,
...
static irqreturn_t dmac_irq_callback(void *ctx, ULONG ulVector)
{
...
//    API_InterVectorSetPriority(KENDRYTE_PLIC_VECTOR(IRQN_DMA0_INTERRUPT + channel_num), priority);
//    API_InterVectorConnect(KENDRYTE_PLIC_VECTOR(IRQN_DMA0_INTERRUPT + channel_num), (PINT_SVR_ROUTINE)dmac_irq_callback,
//                           (PVOID)&dmac_context[channel_num], "dma0_isr");
//    API_InterVectorEnable(KENDRYTE_PLIC_VECTOR(IRQN_DMA0_INTERRUPT + channel_num));
}
