...
/*
 * 裸机库和bspk210兼容性修改(临时处理)
 */
static irqreturn_t real_timer_isr(PVOID parm, ULONG ulVector)
{
    uint32_t timer_number;
...
//        plic_irq_enable(IRQN_TIMER0A_INTERRUPT + timer_number * 2);
        API_InterVectorSetPriority(KENDRYTE_PLIC_VECTOR(IRQN_TIMER0A_INTERRUPT + timer_number * 2), priority);
        API_InterVectorConnect(KENDRYTE_PLIC_VECTOR(IRQN_TIMER0A_INTERRUPT + timer_number * 2), (PINT_SVR_ROUTINE)real_timer_isr,
                               (PVOID)timer[timer_number], "timer_isr");
        API_InterVectorEnable(KENDRYTE_PLIC_VECTOR(IRQN_TIMER0A_INTERRUPT + timer_number * 2));
    }
    else
    {
