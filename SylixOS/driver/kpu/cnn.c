...
irqreturn_t cnn_continue(PVOID _task, ULONG ulVector)
{
    LOG("_start %s\n", __func__);
    cnn_task_t* task = (cnn_task_t*)_task;
...
int cnn_run_dma_input_done_push_layers(void* _task)
{
    ...
    cnn_continue(task, IRQN_AI_INTERRUPT);
...
int cnn_run(cnn_task_t* task, int dma_ch, void *src, void* dst, plic_irq_callback_t cb)
{
    ...
    API_InterVectorEnable(KENDRYTE_PLIC_VECTOR(IRQN_AI_INTERRUPT));
...
void cnn_set_irq(PINT_SVR_ROUTINE cnn_irq, void *ctx)
{
    API_InterVectorSetPriority(KENDRYTE_PLIC_VECTOR(IRQN_AI_INTERRUPT), 4);
    API_InterVectorConnect(KENDRYTE_PLIC_VECTOR(IRQN_AI_INTERRUPT), (PINT_SVR_ROUTINE)cnn_irq,
                           (PVOID)ctx, "cnn_isr");
    API_InterVectorEnable(KENDRYTE_PLIC_VECTOR(IRQN_AI_INTERRUPT));
}
